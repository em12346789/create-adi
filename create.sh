...
#############################################################################
# Generated on Mon Jul 22 07:30:06 2024 by create-alpine-disk-image
...
check_binfmt_packages() {
  local _arch="x86"
  local _binfmt_file="/proc/sys/fs/binfmt_misc/qemu-i386"
...
image_filename="./alpine-v3.18-x86-cloud-aliyun.img"
logfile="./alpine-v3.18-x86-cloud-aliyun.log"
...
write_log "Creating sparse disk image of 151MiB"
truncate -s 151M $image_full_filename >> $logfile
...
write_log "Partitioning disk image for BIOS"
{
  ...
  write_debug_log "Creating 150MiB Root partition" 2
  parted --machine --script --align=optimal $image_full_filename \
...
write_log \
  "Install base Alpine & bootloader packages for x86 arch inside chroot"
{
  _apk_binary="$TMPDIR/apk.static"

  # shellcheck disable=SC2086
  $_apk_binary --arch "x86" --initdb --allow-untrusted \
...
write_log "Doing basic OS configuration"
{
  ...
  printf '\nWelcome\n\n' > /etc/issue
  printf '\n\n%s\n\n' "Alpine x86 aliyun Cloud server" > /etc/motd
